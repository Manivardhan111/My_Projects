...
import { QuestionFreeList } from "../Redux/Action";
import { EachQuestion } from "../Redux/Action";
import { useNavigate } from "react-router-dom";

const EachFreeQuestion = () => {
  const freequestionsList = useSelector((state) => state.questionfreeList);

  const dispatch = useDispatch();
  const navigate = useNavigate();
  useEffect(() => {}, []);
  const handleFreQuestion = (id) => {
    dispatch(EachQuestion(id, navigate));
  };
  return (
    <div className=" text-center bg-dark text-warning pt-5 pb-5">
      <u className="fa fw-bold fs-1 mb-5 ">Select Type of Exam</u>
      <br />

      {freequestionsList.exams ? (
        <div>
          {freequestionsList.exams.map((xam, index) => (
            <div key={index}>
              ...
              {Object.values(xam)[0].map((nme, index) => (
                ...
                >
                  <p
                    ...
                    onClick={() => handleFreQuestion(nme.Exam.id)}
                  >
                    {nme.Exam.name}
                  </p>
                </div>
...
export default EachFreeQuestion;
